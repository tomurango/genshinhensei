...
function get_teams(){
    if(get_flag){
        ...
        // すでに取得しているチームを取らないようにする
        var have_global = getTeamIdsfromGlobal();
        firebase.firestore().collection("teams").where(firebase.firestore.FieldPath.documentId(), 'not-in', have_global).limit(10).get().then(function(teams){
            get_flag = false;
            //loop
            //global_team = teams;
            //console.log("team 数", teams.size);
...
function insert_ateam(team_data, team_id){
    ...
    var team_div = '<div id="' + team_id + '" class="mdc-card team_card" onclick="detail_team(this)"><div class="mdc-card__media mdc-card__media--16-9 team_image lazyload"></div><div class="mdc-card__content team_info_container"><p class="team_name_oncard"></p><p class="team_exp_oncard"></p></div></div>';
